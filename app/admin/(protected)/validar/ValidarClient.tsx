...
import { ExpressQR } from '@/components/admin/ExpressQR'
import { ExpressForm } from '@/components/admin/ExpressForm'
...
export function ValidarClient({
  ...
  return (
    <div className="max-w-lg mx-auto px-4 py-6 flex flex-col gap-6">
      ...
      {!hasResult && !errorMsg && (
        <div className="bg-[#141414] border border-[#1e1e1e] rounded-xl p-5">
          ...
          <p className="text-gray-500 text-xs mt-2">
            Dica: aponte a câmera do celular para o QR Code do cupom do cliente —
            a tela abre com o código já preenchido.
          </p>
        </div>
      )}
      ...
      {/* ─── FLUXO 2: o cliente gera o próprio cupom pelo QR ───
           O formulário express ficou só para o admin, como saída de emergência.
           Ver docs/superpowers/specs/2026-07-28-cupom-express-anti-abuso-design.md */}
      {influencer && !successCoupon && !can(role, 'coupons.express') && (
        <div className="flex flex-col gap-4">
          <ExpressQR
            couponCode={influencer.coupon_code}
            handle={influencer.instagram_handle}
            descontoLabel={formatDiscount(parceriaDo(influencer) ?? {})}
            validityDays={parceriaDo(influencer)?.validity_days ?? 0}
          />
          <button
            onClick={resetAll}
            className="text-gray-500 text-sm text-center hover:text-gray-300 transition-colors py-2"
      ...
      {/* Formulário express — só admin, saída de emergência */}
      {influencer && !successCoupon && can(role, 'coupons.express') && (
        <ExpressForm
